...
import { Button, Dropdown, Icon, IconButton, Modal, TextArea, TextInput } from '../../../components/ui.js';
...
export function GitPanel({ cwd, onClose }: { cwd: string; onClose: () => void }) {
  ...
  return <>
    <aside className={`git-panel${resizing ? ' is-resizing' : ''}`} style={{ width: `${panelWidth}px` }} aria-label="Git">
      ...
      <header className="git-panel-header"><div className="git-panel-title"><GitBranch size={16} /><strong>{snapshot?.repositoryName ?? 'Git'}</strong><span>{currentBranch}</span></div><div className="git-panel-actions"><IconButton icon={RefreshCw} iconSize={15} label="Refresh Git status" onClick={() => void reload()} disabled={busy !== undefined} /><IconButton icon={X} iconSize={16} label="Close Git panel" onClick={onClose} /></div></header>
