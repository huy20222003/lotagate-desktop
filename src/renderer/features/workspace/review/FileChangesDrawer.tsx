...
export function FileChangesDrawer({ cwd, summary, initialExpandedPath, initialFile, onClose }: { cwd: string; summary: FileChangeSummary; initialExpandedPath?: string; initialFile?: OpenFileTarget; onClose: () => void }) {
  ...
  const openedInitialFileRef = useRef<string>();
  ...
  const openFile = useCallback(async (target: OpenFileTarget) => {
    setActiveTab(fileTabValue(target.path));
    if (openFiles[target.path] !== undefined) return;
    setOpenFiles(current => ({ ...current, [target.path]: { status: 'loading' } }));
    try {
      const content = target.artifact === undefined
        ? await window.lotagate.git.readFile(cwd, target.path)
        : (await window.lotagate.tasks.previewArtifact(target.artifact.taskId, target.artifact.artifactId)).content ?? '';
      setOpenFiles(current => ({ ...current, [target.path]: { status: 'ready', content } }));
    } catch (reason) { setOpenFiles(current => ({ ...current, [target.path]: { status: 'error', error: toMessage(reason) } })); }
  }, [cwd, openFiles]);
  useEffect(() => {
    if (initialFile === undefined || initialFileKey === undefined || openedInitialFileRef.current === initialFileKey) return;
    openedInitialFileRef.current = initialFileKey;
