...
export function AgentMarkdown({ content, fileReferences = [], filePaths = [], workspaceCwd }: { content: string; fileReferences?: readonly MessageFileReference[]; filePaths?: readonly string[]; workspaceCwd?: string }) {
  ...
  return <div className="agent-markdown"><Markdown remarkPlugins={[remarkGfm]} components={{ a: SafeLink, pre: MarkdownCodeBlock, p: ({ children }) => <p>{renderMessageChildren(children, references, workspaceCwd)}</p>, li: ({ children }) => <li>{renderMessageChildren(children, references, workspaceCwd)}</li> }}>{protectNestedCodeFences(content)}</Markdown></div>;
}
