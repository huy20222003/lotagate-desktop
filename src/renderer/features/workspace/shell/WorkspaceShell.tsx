...
export function WorkspaceShell({ user, onLoggedOut }: { user: UserProfile; onLoggedOut: () => void }) {
  ...
  const openChanges = useCallback((summary: FileChangeSummary, initialExpandedPath?: string) => { setPlanDrawerOpen(false); setChangesSummary(summary); setChangesInitialExpandedPath(initialExpandedPath); setChangesInitialFile(undefined); setSourcesOpen(false); setBrowserOpen(false); setGitOpen(false); setLiveChatOpen(false); setChangesOpen(true); }, []);
  const openAttachmentFile = useCallback((target: OpenFileTarget) => { setPlanDrawerOpen(false); setChangesSummary({ files: [], additions: 0, deletions: 0 }); setChangesInitialExpandedPath(undefined); setChangesInitialFile(target); setSourcesOpen(false); setBrowserOpen(false); setGitOpen(false); setLiveChatOpen(false); setChangesOpen(true); }, []);
  const openPlan = useCallback(() => { setPlanDrawerOpen(current => !current); setChangesOpen(false); setSourcesOpen(false); setBrowserOpen(false); setGitOpen(false); setLiveChatOpen(false); }, []);
  ...
  const toggleFileChanges = useCallback(() => { if (changesOpen) { setChangesOpen(false); return; } setChangesSummary(latestFileChanges); setChangesInitialExpandedPath(undefined); setChangesInitialFile(undefined); setSourcesOpen(false); setBrowserOpen(false); setGitOpen(false); setLiveChatOpen(false); setChangesOpen(true); }, [changesOpen, latestFileChanges]);
  const toggleSources = useCallback(() => { if (sourcesOpen) { setSourcesOpen(false); return; } setChangesOpen(false); setBrowserOpen(false); setGitOpen(false); setLiveChatOpen(false); setSourcesOpen(true); }, [sourcesOpen]);
  const toggleGit = useCallback(() => { if (gitOpen) { setGitOpen(false); return; } setChangesOpen(false); setSourcesOpen(false); setBrowserOpen(false); setLiveChatOpen(false); setGitOpen(true); }, [gitOpen]);
  const toggleLiveChat = useCallback(() => { if (liveChatOpen) { setLiveChatOpen(false); return; } setChangesOpen(false); setSourcesOpen(false); setBrowserOpen(false); setGitOpen(false); setPlanDrawerOpen(false); setTerminalOpen(false); setLiveChatOpen(true); }, [liveChatOpen]);
  ...
  useEffect(() => { setChangesOpen(false); setSourcesOpen(false); setTerminalOpen(false); setChangesSummary(undefined); setChangesInitialExpandedPath(undefined); setChangesInitialFile(undefined); setPlanDrawerOpen(false); setAgentBrowserSessionId(undefined); setBrowserOpen(false); setGitOpen(false); setLiveChatOpen(false); }, [controller.task?.id]);
  ...
  return <div className={`workspace-shell${sidebarCollapsed ? ' sidebar-collapsed' : ''}`} style={{ gridTemplateColumns: sidebarCollapsed ? '64px minmax(0, 1fr)' : `${sidebarWidth}px minmax(0, 1fr)` }}>
     <WorkspaceSidebar accountName={accountName} avatarProps={avatarProps} workspaces={controller.workspaces} activeWorkspace={controller.workspace} tasks={controller.tasks} activeTask={controller.task} runningTaskIds={controller.runningTaskIds} unreadTaskIds={controller.unreadTaskIds} loading={controller.loading} accountOpen={accountOpen} onAccount={() => setAccountOpen(open => !open)} onCloseAccount={() => setAccountOpen(false)} onSettings={() => { setAutomationOpen(false); setRemoteControlOpen(false); setLiveChatOpen(false); setSettingsSection('profile'); setSettingsOpen(true); setAccountOpen(false); }} onPlugins={() => { setAutomationOpen(false); setRemoteControlOpen(false); setLiveChatOpen(false); setSettingsSection('plugin'); setSettingsOpen(true); setAccountOpen(false); }} onAutomations={() => { setAutomationOpen(true); setRemoteControlOpen(false); setSettingsOpen(false); setAccountOpen(false); setChangesOpen(false); setPlanDrawerOpen(false); setSourcesOpen(false); setBrowserOpen(false); setGitOpen(false); setLiveChatOpen(false); setTerminalOpen(false); }} onRemoteControl={() => { setRemoteControlOpen(true); setAutomationOpen(false); setSettingsOpen(false); setAccountOpen(false); setChangesOpen(false); setPlanDrawerOpen(false); setSourcesOpen(false); setBrowserOpen(false); setGitOpen(false); setLiveChatOpen(false); setTerminalOpen(false); }} onLogout={() => { setLogoutOpen(true); setAccountOpen(false); }} onNewChat={startNewChat} onWorkspace={workspace => { setAutomationOpen(false); setRemoteControlOpen(false); setLiveChatOpen(false); controller.selectWorkspace(workspace); }} onTask={task => { setAutomationOpen(false); setRemoteControlOpen(false); setLiveChatOpen(false); controller.selectTask(task); }} onAddWorkspace={() => void openWorkspacePicker()} onRenameWorkspace={startRename} onRemoveWorkspace={startRemove} onArchiveTask={setArchiveTarget} onPinTask={(target, pinned) => { void controller.pinTaskById(target.id, pinned).catch(reason => showError('Unable to update session pin', toMessage(reason))); }} onRenameTask={startRenameTask} collapsed={sidebarCollapsed} onToggleCollapsed={() => setSidebarCollapsed(current => !current)} sidebarResizing={sidebarResizing} onStartResize={startSidebarResize} onResizeKeyDown={handleSidebarResizeKeyDown} />
    <main className={`conversation${automationOpen || remoteControlOpen ? ' automation-view' : ''}${changesOpen ? ' has-file-changes' : ''}${browserOpen ? ' has-browser' : ''}${gitOpen ? ' has-git' : ''}${liveChatOpen ? ' has-live-chat' : ''}${terminalOpen && terminalPlacement === 'right' ? ' has-terminal-drawer' : ''}${isWelcomeState && !automationOpen && !remoteControlOpen ? ' is-welcome' : ''}`}>
        {automationOpen ? <WorkspaceAutomationPage workspaces={controller.workspaces} /> : remoteControlOpen ? <RemoteControlPage /> : <>
        {isWelcomeState ? null : <ConversationHeader task={controller.task} workspace={controller.workspace} changesOpen={changesOpen} sourcesOpen={sourcesOpen} terminalOpen={terminalOpen} liveChatOpen={liveChatOpen} gitOpen={gitOpen} onToggleChanges={toggleFileChanges} onToggleSources={toggleSources} onToggleTerminal={() => { setLiveChatOpen(false); setTerminalOpen(open => !open); }} onToggleLiveChat={toggleLiveChat} onToggleGit={toggleGit} onRenameTask={startRenameTask} onPinTask={(pinned) => { if (controller.task) void controller.pinTaskById(controller.task.id, pinned).catch(reason => showError('Unable to update session pin', toMessage(reason))); }} onArchiveTask={() => { if (controller.task) setArchiveTarget(controller.task); }} />}
       <div className="conversation-columns">
        ...
        {!isWelcomeState && changesOpen && controller.workspace ? <FileChangesDrawer cwd={controller.workspace.rootPath} summary={changesSummary ?? controller.fileChanges} {...(changesInitialExpandedPath === undefined ? {} : { initialExpandedPath: changesInitialExpandedPath })} {...(changesInitialFile === undefined ? {} : { initialFile: changesInitialFile })} onClose={() => { setChangesOpen(false); setChangesInitialFile(undefined); }} /> : null}
         ...
         {!isWelcomeState && liveChatOpen && controller.workspace ? <LiveChatDrawer cwd={controller.workspace.rootPath} models={controller.models} thinking={controller.thinking} error={controller.error} assistantActivities={controller.activities} activeTurnId={controller.activeTurnId} assistantFinalResponse={controller.assistantFinalResponse} onSend={controller.sendPrompt} onClose={() => setLiveChatOpen(false)} /> : null}
