...
export function WorkspaceShell({ user, onLoggedOut }: { user: UserProfile; onLoggedOut: () => void }) {
  ...
  const openChanges = useCallback((summary: FileChangeSummary, initialExpandedPath?: string) => { setPlanDrawerOpen(false); setChangesSummary(summary); setChangesInitialExpandedPath(initialExpandedPath); setSourcesOpen(false); setBrowserOpen(false); setGitOpen(false); setChangesOpen(true); }, []);
  ...
  const toggleFileChanges = useCallback(() => { if (changesOpen) { setChangesOpen(false); return; } setChangesSummary(latestFileChanges); setChangesInitialExpandedPath(undefined); setSourcesOpen(false); setBrowserOpen(false); setGitOpen(false); setChangesOpen(true); }, [changesOpen, latestFileChanges]);
  ...
  useEffect(() => { setChangesSummary(undefined); setChangesInitialExpandedPath(undefined); setPlanDrawerOpen(false); setAgentBrowserSessionId(undefined); setBrowserOpen(false); setGitOpen(false); }, [controller.task?.id]);
