    ...
    <main className={`conversation${automationOpen || remoteControlOpen ? ' automation-view' : ''}${changesOpen ? ' has-file-changes' : ''}${browserOpen ? ' has-browser' : ''}${gitOpen ? ' has-git' : ''}${liveChatOpen ? ' has-live-chat' : ''}${terminalOpen && terminalPlacement === 'right' ? ' has-terminal-drawer' : ''}${isWelcomeState && !automationOpen && !remoteControlOpen ? ' is-welcome' : ''}`}>
       ...
       <div className="conversation-columns">
        ...
        {!isWelcomeState && changesOpen && controller.workspace ? <FileChangesDrawer cwd={controller.workspace.rootPath} summary={changesSummary ?? controller.fileChanges} {...(changesInitialExpandedPath === undefined ? {} : { initialExpandedPath: changesInitialExpandedPath })} {...(changesInitialFile === undefined ? {} : { initialFile: changesInitialFile })} onClose={() => { setChangesOpen(false); setChangesInitialFile(undefined); }} /> : null}
         ...
         {!isWelcomeState && liveChatOpen && controller.workspace ? <LiveChatDrawer cwd={controller.workspace.rootPath} models={controller.models} thinking={controller.thinking} error={controller.error} assistantFinalResponse={controller.assistantFinalResponse} onSend={controller.sendPrompt} onClose={() => setLiveChatOpen(false)} /> : null}
