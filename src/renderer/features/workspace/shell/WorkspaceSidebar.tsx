...
import { Blocks, ChevronDown, CirclePlus, Clock3, Folder, PanelLeftClose, PanelLeftOpen, Pencil, RadioTower, Settings, X } from 'lucide-react';
...
export interface WorkspaceSidebarProps {
  ...
  accountOpen: boolean; onAccount: () => void; onCloseAccount: () => void; onSettings: () => void; onPlugins: () => void; onAutomations: () => void; onRemoteControl: () => void; onLogout: () => void;
...
export function WorkspaceSidebar({ accountName, avatarProps, workspaces, activeWorkspace, tasks, activeTask, runningTaskIds, unreadTaskIds, loading, accountOpen, onAccount, onCloseAccount, onSettings, onPlugins, onAutomations, onRemoteControl, onLogout, onNewChat, onWorkspace, onTask, onAddWorkspace, onRenameWorkspace, onRemoveWorkspace, onArchiveTask, onPinTask, onRenameTask, collapsed, onToggleCollapsed, sidebarResizing, onStartResize, onResizeKeyDown }: WorkspaceSidebarProps) {
  ...
  return <aside className={`sidebar${sidebarResizing ? ' is-resizing' : ''}`}>
    ...
    <div className="sidebar-primary-actions">
      ...
      <Button variant="ghost" className="sidebar-remote-control sidebar-action" onClick={onRemoteControl}><Icon icon={RadioTower} size={16} /> Remote control</Button>
    </div>
