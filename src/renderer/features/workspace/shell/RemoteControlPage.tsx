...
import { Button, Card, Icon, Skeleton, useToast } from '../../../components/ui.js';
...
export function RemoteControlPage() {
  ...
  const copyLink = async () => { if (!session) return; const copied = await copy(session.connectUrl); if (copied) success('Connection link copied'); else setError('Unable to copy the connection link.'); };
  ...
  return <div className="automation-workspace-page remote-control-workspace-page">
    ...
    <Scrollbar className="automation-workspace-scrollbar remote-control-workspace-scrollbar"><div className="remote-control-content">
      ...
      {activeSession && session.status === 'connected' && qrDataUrl ? <Card className="remote-control-session-card">
        <div className="remote-control-session-layout"><div className="remote-control-qr-panel"><div className="remote-control-qr"><img src={qrDataUrl} alt="Scan to connect to LotaGate Desktop Remote Control" /></div><span><Smartphone size={14} /> Scan with your phone</span></div><div className="remote-control-session-details"><span className="remote-control-eyebrow">PRIVATE SESSION</span><h2>Ready to connect</h2><p>Scan the QR code with your phone. This link is temporary and only works for this Desktop session.</p><div className="remote-control-link"><code>{session.connectUrl}</code><Button variant="ghost" onClick={() => void copyLink()}><Icon icon={copyStatus === 'copied' ? Check : Copy} size={15} /> {copyStatus === 'copied' ? 'Copied' : copyStatus === 'error' ? 'Copy failed' : 'Copy link'}</Button></div><div className="remote-control-session-meta"><span><Clock3 size={14} /> Available until {new Date(session.expiresAt).toLocaleString()}</span><span><ShieldCheck size={14} /> Your Desktop remains the execution authority</span></div>{error ? <p className="remote-control-error" role="alert">{error}</p> : null}<div className="remote-control-actions"><Button variant="danger" disabled={busy} onClick={() => void revokeSession()}><Icon icon={Square} size={14} /> End remote session</Button></div></div></div>
      </Card> : activeSession ? <RemoteSessionSkeleton session={session} error={error} busy={busy} onRevoke={revokeSession} /> : <Card className="remote-control-start-card"><div className="remote-control-start-icon"><Icon icon={RadioTower} size={24} /></div><div className="remote-control-start-copy"><span className="remote-control-eyebrow">PRIVATE SESSION</span><h2>{session && TERMINAL_STATUSES.has(session.status) ? 'Session ended' : 'Control your Desktop from anywhere'}</h2><p>{session && TERMINAL_STATUSES.has(session.status) ? 'Create a new temporary session to connect another device.' : 'Start a secure, temporary connection. Your files, tools and agent stay on this Desktop.'}</p></div><div className="remote-control-steps"><span><b>1</b><span>Create a session</span></span><span><b>2</b><span>Scan the QR code</span></span><span><b>3</b><span>Continue on your phone</span></span></div>{error ? <p className="remote-control-error" role="alert">{error}</p> : null}<div className="remote-control-actions"><Button variant="primary" disabled={busy} onClick={() => void createSession()}><Icon icon={session && TERMINAL_STATUSES.has(session.status) ? Check : RadioTower} size={14} /> {busy ? 'Creating…' : 'Create remote session'}</Button></div></Card>}
    </div></Scrollbar>
  </div>;
}

function RemoteSessionSkeleton({ session, error, busy, onRevoke }: { session: RemoteControlSession | null; error?: string | undefined; busy: boolean; onRevoke: () => Promise<void> }) {
  const connected = session?.status === 'connected';
  return <Card className="remote-control-session-card">
    <div className="remote-control-session-layout"><div className="remote-control-qr-panel"><Skeleton className="remote-control-qr-skeleton" /><span><Smartphone size={14} /> Scan with your phone</span></div><div className="remote-control-session-details"><span className="remote-control-eyebrow">PRIVATE SESSION</span><h2>{connected ? 'Preparing QR code…' : 'Connecting to Desktop…'}</h2><p>{connected ? 'The connection is ready. Generating a QR code for your phone.' : 'LotaGate is establishing the secure Desktop connection. The QR code and link will appear when it is ready.'}</p><div className="remote-control-link"><Skeleton className="remote-control-link-skeleton" /></div><div className="remote-control-session-meta">{session?.expiresAt ? <span><Clock3 size={14} /> Available until {new Date(session.expiresAt).toLocaleString()}</span> : null}<span><ShieldCheck size={14} /> Your Desktop remains the execution authority</span></div>{error ? <p className="remote-control-error" role="alert">{error}</p> : null}<div className="remote-control-actions"><Button variant="danger" disabled={busy} onClick={() => void onRevoke()}><Icon icon={Square} size={14} /> End remote session</Button></div></div></div>
  </Card>;
}
