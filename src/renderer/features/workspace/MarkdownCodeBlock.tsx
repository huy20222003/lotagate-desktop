...
export function MarkdownCodeBlock({ children }: { children?: ReactNode }) {
  ...
  return <div className="markdown-code-block"><header><span>{language}</span><CopyTextButton content={content} label="Copy code" /></header><Scrollbar axis="both" className="markdown-code-scroll"><pre>{children}</pre></Scrollbar></div>;
}
