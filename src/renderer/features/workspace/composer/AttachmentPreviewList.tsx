...
function AttachmentPreviewContent({ taskId, attachments, onRemove, onOpen, className, inline }: { taskId?: string; attachments: readonly AttachmentPreview[]; onRemove?: (attachmentId: string) => Promise<void>; onOpen?: (attachment: AttachmentPreview) => void; className?: string; inline: boolean }) {
  ...
  const previewList = <div className={`attachment-preview-list ${onRemove ? 'composer-attachment-list' : 'user-attachment-list'} ${className ?? ''}`} aria-label="Attached files">{attachments.map(attachment => {
    const url = mediaUrls[attachment.id];
    ...
    return <div className="attachment-preview" key={attachment.id}>{canOpen ? <button type="button" className="attachment-preview-open" aria-label={`Open ${attachment.name}`} onClick={() => openAttachment(attachment)}>{content}</button> : content}{onRemove ? <IconButton icon={X} iconSize={14} className="attachment-remove" label={`Remove ${attachment.name}`} onClick={() => void onRemove(attachment.id)} /> : null}</div>;
  })}</div>;
