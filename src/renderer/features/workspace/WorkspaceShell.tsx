...
export function WorkspaceShell({ user, onLoggedOut }: { user: UserProfile; onLoggedOut: () => void }) {
  ...
  if (settingsOpen) return <SettingsPage user={user} {...(controller.workspace === undefined ? {} : { workspace: controller.workspace })} workspaces={controller.workspaces} mcpStatuses={controller.mcpStatuses} keyboardShortcuts={keyboardShortcuts.bindings} onUpdateShortcut={keyboardShortcuts.updateShortcut} initialSection={settingsSection} onBack={() => setSettingsOpen(false)} />;
  ...
  return <div className={`workspace-shell${sidebarCollapsed ? ' sidebar-collapsed' : ''}`} style={{ gridTemplateColumns: sidebarCollapsed ? '64px minmax(0, 1fr)' : `${sidebarWidth}px minmax(0, 1fr)` }}>
    ...
    <main className={`conversation${changesOpen ? ' has-file-changes' : ''}${browserOpen ? ' has-browser' : ''}${gitOpen ? ' has-git' : ''}${terminalOpen && terminalPlacement === 'right' ? ' has-terminal-drawer' : ''}${isWelcomeState ? ' is-welcome' : ''}`}>
