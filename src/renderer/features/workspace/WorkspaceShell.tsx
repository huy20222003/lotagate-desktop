...
import { useAgentResponseNotifications } from './use-agent-response-notifications.js';
...
export function WorkspaceShell({ user, onLoggedOut }: { user: UserProfile; onLoggedOut: () => void }) {
  ...
  const keyboardShortcuts = useKeyboardShortcuts({
    ...
    openSettings: () => { setAutomationOpen(false); setSettingsOpen(true); setAccountOpen(false); },
  ...
  useEffect(() => { let mounted = true; void window.lotagate.settings.get().then(settings => { if (mounted) setTerminalPlacement(settings.terminalPlacement); }).catch(() => undefined); return () => { mounted = false; }; }, [settingsOpen]);
