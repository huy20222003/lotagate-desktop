...
export function ConversationTimeline({ activities, onSelect, viewportRef }: { activities: Activity[]; onSelect: (activityId: string) => void; viewportRef: RefObject<HTMLDivElement | null> }) {
  ...
  return <aside className="conversation-timeline" aria-label="User messages">{userMessages.map(activity => <button type="button" key={activity.id} className={`conversation-timeline-marker ${activeActivityId === activity.id ? 'active' : ''}`} aria-current={activeActivityId === activity.id ? 'location' : undefined} aria-label={`Jump to message: ${formatTextClamp(80, activity.text)}`} onClick={() => onSelect(activity.id)}><span /><span className="conversation-timeline-popover">{formatTextClamp(120, activity.text)}</span></button>)}</aside>;
}
