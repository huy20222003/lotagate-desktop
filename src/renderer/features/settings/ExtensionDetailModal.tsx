...
import { PluginDetailFields } from './PluginDetailFields.js';
...
export function ExtensionDetailModal({ kind, cwd, row, detail, onClose, onSaved }: { kind: ExtensionKind; cwd: string; row: ExtensionRow; detail: ExtensionDetail; onClose: () => void; onSaved: () => void }) {
  ...
  const save = async () => { const validationError = firstValidationError(validationErrors); if (validationError !== undefined) { setError(validationError); return; } setSaving(true); setError(undefined); try { const nextContent = kind === 'mcp' ? JSON.stringify({ ...(initialObject ?? {}), config: { ...initialMcpConfig, type: mcpType, url: mcpUrl } }, null, 2) : kind === 'plugin' ? JSON.stringify({ ...(initialObject ?? {}), version: pluginVersion, description: pluginDescription }, null, 2) : kind === 'hook' ? JSON.stringify({ event: hookEvent, command: hookCommand, args: parseArgs(hookArgs), timeoutMs: Number(hookTimeout) }, null, 2) : content; await window.lotagate.extensions.writeDetail({ kind, cwd, name: row.name, ...(row.scope === undefined ? {} : { scope: row.scope }), content: nextContent }); onSaved(); } catch (reason) { setError(toMessage(reason)); } finally { setSaving(false); } };
