...
const defaults: SandboxSettings = { ...SANDBOX_DEFAULTS };
...
export function SandboxSettingsPage() {
  ...
  useEffect(() => {
    void window.lotagate.settings.get()
      ...
      .catch(reason => error('Unable to load sandbox settings', reason instanceof Error ? reason.message : 'Please try again.'))
      .finally(() => setLoading(false));
    void window.lotagate.sandbox.health()
      .then(setHealth)
      .catch(reason => error('Unable to inspect sandbox', reason instanceof Error ? reason.message : 'Please try again.'));
  }, [error]);
  ...
  const repair = async (): Promise<void> => {
    ...
    catch (reason) { error('Unable to repair sandbox dependencies', reason instanceof Error ? reason.message : 'Please try again.'); }
    finally { setRepairing(false); }
  };

  const runtimeOptions = [{ value: 'auto', label: 'Auto · native isolated runtime' }, ...(isWindowsHost() ? [{ value: 'wsl2', label: 'WSL2 on Windows' }] : []), { value: 'disabled', label: 'Disabled' }];

  return <div className="appearance-page">
    <section className="appearance-section appearance-settings-card">
      <div className="appearance-section-heading"><div><h2>Sandbox health</h2><p>Runtime and dependencies are checked independently from the saved policy.</p></div><Button type="button" variant="secondary" disabled={repairing} onClick={() => void repair()}>{repairing ? 'Repairing…' : 'Repair sandbox and dependencies'}</Button></div>
      ...
      {health?.runtime.restartRequired ? <SettingRow title="Restart required" description="Windows must restart before the sandbox can be used."><span>Restart Windows, then reopen Desktop.</span></SettingRow> : null}
      {health?.runtime.isolation ? <SettingRow title="Isolation boundary" description="Shows whether the selected backend is a real guest VM or a host OS sandbox."><span>{health.runtime.isolation === 'vm' ? 'Virtual machine guest' : 'Host OS sandbox'}</span></SettingRow> : null}
      ...
      {health?.dependencies.missing.length ? <SettingRow title="Missing dependencies" description="These manifest entries can be installed for the selected sandbox backend."><span>{health.dependencies.missing.join(', ')}</span></SettingRow> : null}
      ...
      {health?.dependencies.failed.length ? <SettingRow title="Failed repair" description="The last dependency repair did not complete. Resolve the runtime reason, then retry repair."><span>{health.dependencies.failed.join(', ')}</span></SettingRow> : null}
    </section>

    <section className="appearance-section appearance-settings-card">
      <div className="appearance-section-heading"><div><h2>Sandbox runtime</h2><p>Agent filesystem, shell, git, and Python document workflows run inside this isolated guest.</p></div></div>
      <SettingRow title="Runtime" description="Desktop selects WSL2 on Windows, bubblewrap on Linux, and Seatbelt on macOS."><Dropdown ariaLabel="Sandbox runtime" value={value.runtime} options={runtimeOptions} onChange={next => set('runtime', next as SandboxSettings['runtime'])} /></SettingRow>
      {isWindowsHost() ? <SettingRow title="Guest distribution" description="The WSL2 distribution used by the shared Windows guest."><TextInput aria-label="Guest distribution" value={value.distribution} onChange={event => set('distribution', event.target.value)} /></SettingRow> : null}
      <SettingRow title="Workspace access" description="Limit the isolated workspace to read-only or read-write operations."><Dropdown ariaLabel="Sandbox workspace access" value={value.workspaceAccess} options={[{ value: 'read-only', label: 'Read-only' }, { value: 'read-write', label: 'Read and write' }]} onChange={next => set('workspaceAccess', next as SandboxSettings['workspaceAccess'])} /></SettingRow>
    </section>
    ...
    <section className="appearance-section appearance-settings-card">
      ...
      <SettingRow title="Guest network" description="No network is the secure default. Full network is available only when the task requires guest egress."><Dropdown ariaLabel="Sandbox network policy" value={value.networkPolicy} options={[{ value: 'none', label: 'No network' }, { value: 'full', label: 'Full network' }]} onChange={next => set('networkPolicy', next as SandboxSettings['networkPolicy'])} /></SettingRow>
    </section>
    ...
    <section className="appearance-section appearance-settings-card">
      <div className="appearance-section-heading"><div><h2>Fallback and lifecycle</h2><p>Computer Use and the Terminal drawer remain host-native and are not controlled by this sandbox policy.</p></div></div>
      <SettingRow title="Host fallback" description="Choose what happens when the sandbox is unavailable."><Dropdown ariaLabel="Sandbox host fallback" value={value.hostFallback} options={[{ value: 'ask', label: 'Ask for approval' }, { value: 'allow', label: 'Allow host fallback' }, { value: 'deny', label: 'Deny action' }]} onChange={next => set('hostFallback', next as SandboxSettings['hostFallback'])} /></SettingRow>
      ...
      <NumericRow title="Diagnostics retention" description="Keep sandbox lifecycle and execution diagnostics for this many days." label="Diagnostics retention (days)" value={value.diagnosticsRetentionDays} min={SANDBOX_LIMITS.diagnosticsRetentionDays.min} max={SANDBOX_LIMITS.diagnosticsRetentionDays.max} onChange={next => set('diagnosticsRetentionDays', next)} />
    </section>
  </div>;
}
...
function healthDescription(health: SandboxHealthSnapshot | undefined): string {
  ...
  if (health.state === 'disabled') return 'The sandbox is disabled in Settings.';
  if (health.runtime.reason !== undefined) return health.runtime.reason;
  const runtime = health.runtime.runtime === 'unsupported' ? 'No sandbox backend' : health.runtime.runtime === 'wsl2' ? `WSL2 ${health.runtime.distribution}` : health.runtime.runtime === 'bubblewrap' ? 'Linux host sandbox' : 'macOS host sandbox';
  return `${runtime} · checked ${new Date(health.checkedAt).toLocaleTimeString()}.`;
}

function isWindowsHost(): boolean {
  return typeof navigator !== 'undefined' && (/win/iu.test(navigator.platform) || /windows/iu.test(navigator.userAgent));
}
