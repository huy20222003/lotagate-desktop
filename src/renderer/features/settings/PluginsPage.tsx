...
import { Bot, Download, ExternalLink, PackageOpen, Plus, Search, Trash2 } from 'lucide-react';
...
type PluginSource = { name: string; displayName?: string | undefined; version: string; description: string; icon?: string | undefined; scope?: 'user' | 'project' | undefined; author?: string | undefined; license?: string | undefined; homepage?: string | undefined; privacyPolicy?: string | undefined; termsOfService?: string | undefined };
...
const CONTRIBUTION_META = {
  skill: { label: 'Skills', icon: DEFAULT_EXTENSION_ICONS.skill },
  mcp: { label: 'MCP', icon: DEFAULT_EXTENSION_ICONS.mcp },
  hook: { label: 'Hooks', icon: DEFAULT_EXTENSION_ICONS.hook },
...
export function PluginsPage({ cwd, trusted = false, detailResetToken, onDetailChange }: { cwd?: string; trusted?: boolean; detailResetToken?: number; onDetailChange?: (pluginName: string | undefined) => void }) {
  ...
  const publicPlugins = useMemo(() => publicCatalog.filter(plugin => !query || `${plugin.displayName ?? ''} ${plugin.name} ${plugin.description} ${plugin.author ?? ''}`.toLocaleLowerCase().includes(query)), [publicCatalog, query]);
  const personalPlugins = useMemo(() => rows.filter(row => !publicPluginNames.has(row.name) && (!query || `${row.displayName ?? ''} ${row.name} ${row.description ?? ''} ${row.detail}`.toLocaleLowerCase().includes(query))), [publicPluginNames, query, rows]);
  ...
  const installPublic = useCallback(async (plugin: PublicPluginCatalogEntry): Promise<boolean> => {
    ...
    try {
      ...
      return await execute({ actionId: 'plugin.install', positionals: [source], options: { scope: 'user' } }, `Installed ${plugin.displayName ?? plugin.name}`);
    } catch (reason) {
      showError('Plugin operation failed', toMessage(reason));
      return false;
    }
  ...
  const openPublicPlugin = useCallback((plugin: PublicPluginCatalogEntry) => {
    ...
    const detail: PluginDetail = {
      plugin: { name: plugin.name, ...(plugin.displayName === undefined ? {} : { displayName: plugin.displayName }), version: plugin.version, description: plugin.description, ...(plugin.author === undefined ? {} : { author: plugin.author }), ...(plugin.license === undefined ? {} : { license: plugin.license }), ...(plugin.homepage === undefined ? {} : { homepage: plugin.homepage }), scope: 'user', status: 'DISABLED' },
  ...
  const installOrUninstall = (plugin: PublicPluginCatalogEntry) => {
    ...
    void execute({ actionId: 'plugin.uninstall', positionals: [plugin.name], options: { scope: row.scope } }, `Uninstalled ${plugin.displayName ?? plugin.name}`);
  };
  ...
  const uninstallSelected = () => {
    ...
    void execute({ actionId: 'plugin.uninstall', positionals: [selected.detail.plugin.name], options: { scope: selected.detail.plugin.scope } }, `Uninstalled ${selected.detail.plugin.displayName ?? selected.detail.plugin.name}`).then(done => { if (done) setSelected(undefined); });
  };
...
function PluginDetailView({ selected, cwd, busy, onInstall, onUninstall, onOpenContribution }: { selected: SelectedPlugin; cwd: string | undefined; busy: string | undefined; onInstall: (() => void) | undefined; onUninstall: (() => void) | undefined; onOpenContribution: (contribution: PluginContribution) => void }) {
  ...
  const metadata = detail?.plugin ?? { name: source.name, ...(source.displayName === undefined ? {} : { displayName: source.displayName }), version: source.version, description: source.description, scope: source.scope ?? 'user', status: 'DISABLED' as const };
...
function PluginIcon({ cwd, name, scope, source, size = 'card' }: { cwd: string | undefined; name: string; scope: 'user' | 'project' | undefined; source: string | undefined; size?: 'card' | 'hero' }) {
  ...
  return <span className={`settings-plugin-icon settings-plugin-icon-${size}`}>{icon ? <img src={icon} alt="" /> : <img src={DEFAULT_EXTENSION_ICONS.plugin} alt="" />}</span>;
}

function ContributionIcon({ kind }: { kind: keyof typeof CONTRIBUTION_META }) {
  const icon = CONTRIBUTION_META[kind].icon;
  return typeof icon === 'string' ? <img className="settings-extension-detail-icon" src={icon} alt="" /> : <Icon icon={icon} size={17} />;
}
