...
describe('workspace model catalog', () => {
  it('preserves the CLI model category for command-specific selection', () => {
    expect(extractWorkspaceModels({ models: [{ id: 'model-1', label: 'Model 1', model_category: 'image', context_window: 128_000 }] })).toEqual([{ id: 'model-1', label: 'Model 1', category: 'image' }]);
  });
