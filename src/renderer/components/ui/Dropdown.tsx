...
export function Dropdown(props: DropdownProps) {
  ...
  const items = options.map(option => multiple
    ? <DropdownMenuPrimitive.CheckboxItem className={cn('dropdown-option', 'ui-menu-item')} checked={selectedValues.includes(option.value)} onSelect={event => { event.preventDefault(); toggleValue(option.value); }} key={option.value}><DropdownMenuPrimitive.ItemIndicator className="ui-menu-indicator"><Check size={13} /></DropdownMenuPrimitive.ItemIndicator>{option.label}</DropdownMenuPrimitive.CheckboxItem>
    : <DropdownMenuPrimitive.RadioItem className={cn('dropdown-option', 'ui-menu-item')} value={option.value} key={option.value}><DropdownMenuPrimitive.ItemIndicator className="ui-menu-indicator"><Check size={13} /></DropdownMenuPrimitive.ItemIndicator>{option.label}</DropdownMenuPrimitive.RadioItem>);
  return <DropdownMenuPrimitive.Root open={open} onOpenChange={setOpen}><div className={cn('dropdown', open ? 'open' : '', className)}>{label ? <Label>{label}</Label> : null}<DropdownMenuPrimitive.Trigger asChild><button type="button" className={cn('model-select', 'ui-select-trigger')} aria-label={ariaLabel ?? label ?? 'Select option'} disabled={disabled}><span>{selected}</span><ChevronDown size={14} /></button></DropdownMenuPrimitive.Trigger></div><DropdownMenuPrimitive.Portal><DropdownMenuPrimitive.Content className={cn('dropdown-menu', 'ui-menu-content')} align="start" side="bottom" sideOffset={8} avoidCollisions collisionPadding={8}><Scrollbar className="dropdown-scrollbar">{multiple ? items : <DropdownMenuPrimitive.RadioGroup {...(hasSelectedValue ? { value: singleValue } : {})} onValueChange={toggleValue}>{items}</DropdownMenuPrimitive.RadioGroup>}</Scrollbar></DropdownMenuPrimitive.Content></DropdownMenuPrimitive.Portal></DropdownMenuPrimitive.Root>;
}
