...
export function Dropdown(props: DropdownProps) {
  ...
  return <DropdownMenuPrimitive.Root modal={false} open={open} onOpenChange={setOpen}><div className={cn('dropdown', open ? 'open' : '', className)}>{label ? <Label>{label}</Label> : null}<DropdownMenuPrimitive.Trigger asChild><button type="button" className={cn('model-select', 'ui-select-trigger')} aria-label={ariaLabel ?? label ?? 'Select option'} disabled={disabled}><span>{selected}</span><Icon icon={ChevronDown} size={14} /></button></DropdownMenuPrimitive.Trigger></div><DropdownMenuPrimitive.Portal><DropdownMenuPrimitive.Content className={cn('dropdown-menu', 'ui-menu-content')} align="start" side="bottom" sideOffset={8} avoidCollisions collisionPadding={8}><Scrollbar className="dropdown-scrollbar">{content}</Scrollbar></DropdownMenuPrimitive.Content></DropdownMenuPrimitive.Portal></DropdownMenuPrimitive.Root>;
}
