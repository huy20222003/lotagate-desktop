...
export function Tabs({ value, items, onChange, onClose, ariaLabel = 'Views' }: { value: string; items: Array<{ value: string; label: string }>; onChange: (value: string) => void; onClose?: (value: string) => void; ariaLabel?: string }) {
  return <TabsPrimitive.Root value={value} onValueChange={onChange}><TabsPrimitive.List className={cn('tabs', 'ui-tabs')} aria-label={ariaLabel}>{items.map(item => <span className="tab-item" key={item.value}><TabsPrimitive.Trigger type="button" value={item.value} className={cn('tab', 'ui-tab')}><span>{item.label}</span></TabsPrimitive.Trigger>{onClose ? <button type="button" className="tab-close" aria-label={`Close ${item.label}`} onClick={() => onClose(item.value)}><X size={12} /></button> : null}</span>)}</TabsPrimitive.List></TabsPrimitive.Root>;
}
