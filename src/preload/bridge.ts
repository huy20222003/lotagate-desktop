...
const bridge: DesktopBridge = {
  ...
  agent: {
    ...
    uploadAttachment: (cwd, input) => ipcRenderer.invoke('agent.uploadAttachment', cwd, input),
    deleteAttachment: (cwd, input) => ipcRenderer.invoke('agent.deleteAttachment', cwd, input),
    turnClaim: (cwd, taskId, sessionId) => ipcRenderer.invoke('agent.turnClaim', cwd, taskId, sessionId),
    turnRelease: (taskId, token) => ipcRenderer.invoke('agent.turnRelease', taskId, token),
  ...
  tasks: {
    ...
    queuedPrompts: taskId => ipcRenderer.invoke('task.queuedPrompts', taskId),
    queuePrompt: (taskId, input) => ipcRenderer.invoke('task.queuePrompt', taskId, input),
    dequeuePrompt: (taskId, promptId) => ipcRenderer.invoke('task.dequeuePrompt', taskId, promptId),
    ...
    createTextArtifact: (taskId, name, content, kind, source) => ipcRenderer.invoke('task.createTextArtifact', taskId, name, content, kind, source),
  ...
  approvals: {
    ...
    respond: (approvalId, decision, owner) => ipcRenderer.invoke('approval.respond', approvalId, decision, owner),
  ...
  operations: { notify: (title, body) => ipcRenderer.invoke('operations.notify', title, body), showWindow: () => ipcRenderer.invoke('operations.showWindow'), openExternal: url => ipcRenderer.invoke('operations.openExternal', url), revealPath: path => ipcRenderer.invoke('operations.revealPath', path), openFile: (path, destination) => ipcRenderer.invoke('operations.openFile', path, destination), listDirectory: path => ipcRenderer.invoke('operations.listDirectory', path), readFile: path => ipcRenderer.invoke('operations.readFile', path), previewFile: path => ipcRenderer.invoke('operations.previewFile', path), exportDiagnostics: () => ipcRenderer.invoke('operations.exportDiagnostics'), onDeepLink: listener => { const handler = (_event: Electron.IpcRendererEvent, url: string) => listener(url); ipcRenderer.on('operations.deepLink', handler); return () => ipcRenderer.removeListener('operations.deepLink', handler); } },
  speech: { transcribe: (audio, language) => ipcRenderer.invoke('speech.transcribe', audio, language), synthesize: (cwd, input) => ipcRenderer.invoke('speech.synthesize', cwd, input) },
};
