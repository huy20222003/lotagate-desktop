...
const automationListSchema = automationSchema.array();
export type Automation = z.infer<typeof automationSchema>;
export type AutomationRunner = (automation: Automation) => Promise<void>;

export interface AutomationStore {
  read(): Promise<Automation[]>;
  update(mutator: (current: Automation[]) => Automation[] | Promise<Automation[]>): Promise<Automation[]>;
}
...
export class AutomationService {
  ...
  constructor(store: AutomationStore = new JsonFileStore<Automation[]>(desktopDataPath('automations.json'), [], value => automationListSchema.parse(value))) {
    this.store = store;
  }
  ...
  async run(id: string, runner: AutomationRunner): Promise<Automation> {
    if (this.running.has(id)) throw new Error('Automation is already running.');
    this.running.add(id);
    try {
      const item = (await this.store.read()).find(value => value.id === id);
      if (item === undefined) throw new Error('Automation was not found.');
      if (!item.enabled) throw new Error('Automation is disabled.');
      try { await runner(item); return this.recordRun(item, null); }
      catch (error) { return this.recordRun(item, error instanceof Error ? error.message : 'Automation failed.'); }
    } finally {
      this.running.delete(id);
    }
  }

  private async runDue(runner: AutomationRunner): Promise<void> { const now = Date.now(); for (const item of await this.store.read()) if (!this.running.has(item.id) && item.enabled && item.nextRunAt !== null && Date.parse(item.nextRunAt) <= now) await this.run(item.id, runner); }
