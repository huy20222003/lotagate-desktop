...
type BrowserEntry = { window: BrowserWindow; evidence: BrowserEvidence; recordingTimer: ReturnType<typeof setInterval> | undefined };
...
export class BrowserService {
  ...
  async open(url: string, approved: boolean): Promise<{ id: string; url: string }> {
    ...
    this.windows.set(id, { window: browser, evidence, recordingTimer: undefined });
    ...
    browser.webContents.on('console-message', (_event, level, message) => { evidence.console.push({ level: consoleLevel(level), message: redact(message), timestamp: new Date().toISOString() }); });
    browser.webContents.on('did-fail-load', (_event, errorCode, errorDescription) => { evidence.errors.push(redact(`${errorCode}: ${errorDescription}`)); });
    browser.webContents.on('render-process-gone', (_event, details) => { evidence.errors.push(redact(`Render process ended: ${details.reason}`)); });
    browser.webContents.session.on('will-download', (_event, item) => { item.cancel(); evidence.errors.push('Downloads require an explicit approved desktop download action.'); });
  ...
  private async captureRecordingFrame(entry: BrowserEntry): Promise<void> { if (entry.evidence.recordings.length >= 30) { if (entry.recordingTimer) clearInterval(entry.recordingTimer); entry.recordingTimer = undefined; return; } const { path } = await this.capture(entry); entry.evidence.recordings.push(path); }
