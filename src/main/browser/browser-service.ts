...
export class BrowserService {
  ...
  private applyTabVisibility(entry: BrowserSessionEntry): void { for (const [id, tab] of entry.tabs) { const visible = id === entry.activeTabId && tab.snapshot.url !== 'about:blank' && this.hostWindow !== undefined; tab.view.setVisible(visible); if (!visible) { tab.view.setBounds(EMPTY_BOUNDS); tab.nativeBounds = undefined; } } }
