...
describe('TaskEventProjector turn lifecycle', () => {
  it('persists the turn id as soon as the CLI reports turn.started', async () => {
    ...
    await projector.apply('C:\\workspace', { version: 3, type: 'event', scope: 'session', event: 'turn.started', data: { sessionId: 'session-1', turnId: 'turn-1' } });
  ...
  it('clears stale turn state after a failed turn', async () => {
    ...
    await projector.apply('C:\\workspace', { version: 3, type: 'event', scope: 'session', event: 'turn.failed', data: { sessionId: 'session-1', turnId: 'turn-1', error: 'Agent failed.' } });
  ...
  it('persists the compacted marker emitted by the CLI context manager', async () => {
    const { projector, tasks } = createProjector(createTask());
    await projector.apply('C:\\workspace', { version: 3, type: 'event', scope: 'session', event: 'context.compacted', data: { sessionId: 'session-1', turnId: 'turn-1' } });
  ...
  it('persists tool progress outside the conversation transcript', async () => {
    const { projector, tasks } = createProjector(createTask());
    await projector.apply('C:\\workspace', { version: 3, type: 'event', scope: 'session', event: 'tool.completed', data: { sessionId: 'session-1', toolName: 'browser.newTab', displayName: 'browser.newTab', isError: true } });
  ...
  it('does not duplicate generic lifecycle entries for detailed filesystem activity', async () => {
    const { projector, tasks } = createProjector(createTask());
    await projector.apply('C:\\workspace', { version: 3, type: 'event', scope: 'session', event: 'tool.completed', data: { sessionId: 'session-1', actionId: 'run-1:filesystem.read', toolName: 'filesystem.read', displayName: 'filesystem.read', isError: false } });
    expect(tasks.appendEvent).not.toHaveBeenCalled();
  });

  it('persists shell activity updates with the safe display command', async () => {
    const { projector, tasks } = createProjector(createTask());
    await projector.apply('C:\\workspace', { version: 3, type: 'event', scope: 'session', event: 'tool.activity.started', data: { sessionId: 'session-1', actionId: 'run-1:shell.exec', toolName: 'shell.exec', command: 'Get-Content test.md', status: 'running' } });
  ...
  it('finalizes a persisted assistant segment without adding transcript text', async () => {
    const { projector, tasks } = createProjector(createTask());
    await projector.apply('C:\\workspace', { version: 3, type: 'event', scope: 'session', event: 'assistant.segment.completed', data: { sessionId: 'session-1', turnId: 'turn-1', segmentId: 'run-1:1', phase: 'final' } });
  ...
  it('does not persist command output in the conversation transcript', async () => {
    const { projector, tasks } = createProjector(createTask());
    await projector.apply('C:\\workspace', { version: 3, type: 'event', scope: 'session', event: 'turn.started', data: { sessionId: 'session-1', turnId: 'turn-1' } });
    await projector.apply('C:\\workspace', { version: 3, type: 'event', scope: 'control', event: 'command.output', data: { content: 'command output' } });
  ...
  it('batches assistant deltas and flushes them before the next lifecycle event', async () => {
    const { projector, tasks } = createProjector(createTask());
    await projector.apply('C:\\workspace', { version: 3, type: 'event', scope: 'session', event: 'assistant.delta', data: { sessionId: 'session-1', turnId: 'turn-1', segmentId: 'run-1:1', content: 'Hello ' } });
    await projector.apply('C:\\workspace', { version: 3, type: 'event', scope: 'session', event: 'assistant.delta', data: { sessionId: 'session-1', turnId: 'turn-1', segmentId: 'run-1:1', content: 'world' } });
  ...
  it('does not fall back to the latest cwd task for an event from an unknown session', async () => {
    const { projector, tasks } = createProjector(createTask());
    await projector.apply('C:\\workspace', { version: 3, type: 'event', scope: 'control', event: 'command.output', data: { sessionId: 'unknown-session', content: 'unrelated output' } });
    expect(tasks.appendEvent).not.toHaveBeenCalled();
  ...
  it('uses the explicit task identity before session or cwd fallbacks', async () => {
    const { projector, tasks } = createProjector(createTask());
    await projector.apply('C:\\workspace', { version: 3, type: 'event', scope: 'session', event: 'tool.completed', data: { taskId: 'task-1', sessionId: 'unknown-session', toolName: 'filesystem.list', isError: false } });
