...
const { sockets } = vi.hoisted(() => ({ sockets: [] as Array<{ url: URL; readyState: number; sent: string[]; open: () => void; message: (data: string) => void; close: (code?: number) => void }> }));
...
describe('RemoteControlService', () => {
  ...
  it('creates a relay session without exposing the host credential to the renderer', async () => {
    ...
    sockets[0]!.open();
    expect(sockets[0]!.url.pathname).toBe('/api/v1/ws');
    ...
    expect(fetchMock.mock.calls[1]?.[0]).toBe('https://remote.example/api/v1/sessions/11111111-1111-4111-8111-111111111111');
