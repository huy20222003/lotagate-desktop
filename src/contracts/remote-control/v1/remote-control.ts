...
import type { RemoteSlashCommandDefinition } from './slash-command-catalog.js';
...
export const REMOTE_MEDIA_READ_CHUNK_BYTES = 48 * 1024;
...
export const remoteCommandSchema = z.discriminatedUnion('action', [
  ...
  z.object({ action: z.literal('prompt'), taskId: z.string().min(1).max(256), prompt: z.string().min(1).max(512 * 1024), agentPrompt: z.string().min(1).max(512 * 1024).optional(), model: z.string().min(1).max(256).optional(), skills: z.array(z.string().min(1).max(256)).max(16).optional(), attachmentIds: z.array(z.string().min(1).max(256)).max(16).optional() }).strict(),
  ...
  z.object({ action: z.literal('artifact.read'), taskId: z.string().min(1).max(256), artifactId: z.string().min(1).max(256), offset: z.number().int().nonnegative(), length: z.number().int().positive().max(REMOTE_MEDIA_READ_CHUNK_BYTES) }).strict(),
  ...
  z.object({ action: z.literal('command.execute'), taskId: z.string().min(1).max(256), input: z.object({ actionId: z.string().min(1).max(256), positionals: z.array(z.string().max(16_384)).max(128), options: z.record(z.string(), z.union([z.string().max(16_384), z.boolean()])), attachments: z.record(z.string(), z.array(z.string().min(1).max(256)).max(16)).refine(value => Object.keys(value).length <= 32).optional() }).strict() }).strict(),
]);
...
export interface RemoteTaskSummary { id: string; title: string; status: TaskStatus; sessionId?: string; turnId?: string; updatedAt: string; pinned: boolean }
...
export interface RemoteCommandArgument { name: string; required: boolean; variadic?: boolean }
...
export function remoteTaskSummary(task: Task): RemoteTaskSummary {
  return { id: task.id, title: task.title, status: task.status, ...(task.sessionId === undefined ? {} : { sessionId: task.sessionId }), ...(task.turnId === undefined ? {} : { turnId: task.turnId }), updatedAt: task.updatedAt, pinned: task.pinned };
}
