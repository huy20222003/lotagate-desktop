...
export const hookCreateInputSchema = z.object({ cwd: z.string().min(1).max(4_096), event: hookEventSchema, command: z.string().trim().min(1).max(512), args: z.array(z.string().max(16_384)).max(128), timeoutMs: z.number().int().min(100).max(120_000) });
